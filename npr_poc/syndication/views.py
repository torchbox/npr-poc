...
from .pbs_utils import search_shows, show_to_dict
...
class BrowseSyndicatedContentView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        stories = search_stories(
            search_term=search_query, org=self.request.GET.get("org")
        )
        ctx["stories"] = [story_to_dict(story) for story in stories]
        ctx["search_form"] = SearchForm(
            self.request.GET if search_query else None,
            placeholder="Search for a story, podcast or station",
        )
        ctx["orgs"] = ORGS
...
class BrowsePBSContentView(TemplateView):
    model_admin = None
    template_name = "wagtailadmin/pages/browse_pbs_shows.html"
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        search_query = self.request.GET.get("q")
        shows = search_shows(search_term=search_query)
        ctx["shows"] = [show_to_dict(show) for show in shows]
        ctx["search_form"] = SearchForm(
            self.request.GET if search_query else None,
            placeholder="Search for a PBS show",
        )
        return ctx
...
class SyndicatedContentSearchView(View):
    def get(self, request, *args, **kwargs):
        stories = search_stories(
            search_term=request.GET.get("q"), org=request.GET.get("org")
        )
        return render(
            request,
            "wagtailadmin/pages/syndicated_content_results.html",
            {
...
class PBSSearchView(View):
    def get(self, request, *args, **kwargs):
        shows = search_shows(search_term=request.GET.get("q"))
        return render(
            request,
            "wagtailadmin/pages/pbs_show_results.html",
            {
