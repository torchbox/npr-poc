...
class PodcastFeed(feedgenerator.Rss201rev2Feed):
    ...
    def add_root_elements(self, handler):
        ...
        if self.feed['image']:
            ...
            handler.addQuickElement('url', absurl(self.feed['image'].image.get_rendition('max-144x400').url))
            ...
            handler.addQuickElement(
                'itunes:image', '', {'href': absurl(self.feed['image'].image.get_rendition('max-3000x3000').url)}
            )
        ...
        handler.addQuickElement('itunes:explicit', 'true' if self.feed['explicit'] else 'false')
        handler.addQuickElement('itunes:block', 'Yes' if self.feed['block'] else 'No')
        handler.addQuickElement('itunes:complete', 'Yes' if self.feed['complete']else 'No')
    ...
    def add_item_elements(self, handler, item):
        ...
        if item['image']:
            handler.addQuickElement(
                'itunes:image', '', {'href': absurl(item['image'].image.get_rendition('max-3000x3000').url)}
            )

        handler.addQuickElement('itunes:explicit', 'true' if item['explicit'] else 'false')
...
class ShowFeed(Feed):
    ...
    def item_enclosures(self, item):
        ...
        for enc in item.enclosures.all():
            enclosure = feedgenerator.Enclosure(
                url='{}{}'.format(self.request.build_absolute_uri('/').rstrip('/'), enc.media.file.url),
                length=str(enc.media.file.size),
                mime_type=enc.media.mime_type,
            )
    ...
    def feed_extra_kwargs(self, obj):
        return {
            ...
            'type': 'serial' if obj.podcast_type == obj.SHOW_TYPE_SERLIALIZED else 'episodic',
